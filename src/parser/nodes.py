...
class FunctionDefinitionNode(AbstractSyntaxTreeNode):
    def __init__(self, token: Token, parent: Node, arguments: list[Node], body: Node):
        super().__init__(parent, token)
        self.arguments = arguments
        self.body = body

class IfStatementNode(AbstractSyntaxTreeNode):
    def __init__(self, token: Token, parent: Node, condition: Node, then_branch: Node, else_branch: Node):
        super().__init__(parent, token)
